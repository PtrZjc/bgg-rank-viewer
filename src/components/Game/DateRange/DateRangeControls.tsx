...
export const DateRangeControls: React.FC = () => {
  const datapointNumberVisible = useDateStore(
    (state) => state.datapointNumberVisible
  );
  const setDatapointNumberVisible = useDateStore(
    (state) => state.setDatapointNumberVisible
  );
  ...
  const [inFormDatapointNumber, setInFormDatapointNumber] = useState(
    datapointNumberVisible
  );
  const [inFormTopRanksShowed, setInFormTopRanksShowed] =
    useState(topRanksShowed);

  const debouncedSetDatapoints = useDebouncedCallback(
    (value: number) => setDatapointNumberVisible(value),
    300
  );

  const debouncedSetTopRanks = useDebouncedCallback(
    (value: number) => setTopRanksShowed(value),
    300
  );

  const updateTopRanks = useCallback(
    (target: number) => {
  ...
  const updateDatapoints = useCallback(
    (target: number) => {
      ...
      if (target > DAYS_FROM_ZERO_DATE_TO_TODAY)
        target = DAYS_FROM_ZERO_DATE_TO_TODAY;
